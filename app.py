...
import pandas as pd
import requests
from datetime import datetimest.set_page_config(page_title="My 20-Stock Valuation Dashboard", layout="wide")
st.title("My Personal 20-Year EV/EBITDA & P/S Watchlist")
st.markdown("**Green cells** = currently cheaper than 20-year historical average")# ==================== YOUR FREE API KEY ====================
# Get it in 10 seconds here: https://site.financialmodelingprep.com/developer/docs
FMP_KEY = "g4ZuHAQ1NJMzQXnSc6ALNTEsKjxJS0gC"          # ←←← PUT YOUR KEY GOES HEREif FMP_KEY == "YOUR_FMP_KEY_HERE":
    st.error("Please get your free FMP API key and paste it in the code (line above)")
    st.stop()# ==================== CUSTOM WATCHLIST INPUT ====================
st.sidebar.header("My Watchlist (max 30 symbols)")
default_list = "AAPL MSFT GOOGL AMZN NVDA TSLA META BRK-B JNJ JPM V PG UNH HD MA DIS NFLX ADBE PYPL"user_input = st.sidebar.text_area(
...
)# Clean and limit to 30 symbols
raw_tickers = [x.strip().upper() for x in user_input.replace(",", " ").split() if x.strip()]
MY_TICKERS = list(dict.fromkeys(raw_tickers))[:30]      # remove duplicates, max 30# Save for next session
st.session_state.my_tickers = " ".join(MY_TICKERS)st.sidebar.success(f"Tracking {len(MY_TICKERS)} stocks")# ==================== FETCH CURRENT RATIOS ====================
@st.cache_data(ttl=3600, show_spinner=False)
def get_current_ratios(tickers):
    rows = []
    for symbol in tickers:
        try:
            # Try yfinance first (fast)
            import yfinance as yf
            info = yf.Ticker(symbol).info
            ev = info.get("enterpriseToEbitda")
            ps = info.get("priceToSalesTrailing12Months")
            name = info.get("longName", symbol)        # FMP fallback if missing
...
return pd.DataFrame(rows)# ==================== 20-YEAR HISTORICAL AVERAGES ====================
    ...
    for symbol in tickers:
        try:
            ...
            if not yearly:
                ...
                continue        df = pd.DataFrame(yearly)
        ...
        rows.append({"Symbol": symbol, "20Y Avg EV/EBITDA": None, "20Y Avg P/S": None, "Years": 0})
return pd.DataFrame(rows)# ==================== LOAD AND MERGE ====================
with st.spinner("Loading current ratios..."):
    current_df = get_current_ratios(MY_TICKERS)with st.spinner("Calculating 20-year historical averages..."):
    hist_df = get_20y_averages(MY_TICKERS)df = current_df.merge(hist_df, on="Symbol")# ==================== HIGHLIGHTING ====================
def highlight(row):
    styles = [""] * len(row)
    ...
    return stylesstyled = df.style\
    .apply(highlight, axis=1)\
    .format({
        "EV/EBITDA": "{:.2f}", "20Y Avg EV/EBITDA": "{:.2f}",
        "P/S": "{:.2f}", "20Y Avg P/S": "{:.2f}"
    }, na_rep="—")\
    .set_properties(**{"text-align": "center"}, subset=["EV/EBITDA","20Y Avg EV/EBITDA","P/S","20Y Avg P/S"])st.dataframe(styled, use_container_width=True, height=900)# Summary
cheap = df[
    ((df["EV/EBITDA"] < df["20Y Avg EV/EBITDA"]) | (df["P/S"] < df["20Y Avg P/S"]))]
st.success(f"**{len(cheap)} out of {len(df)}** stocks are currently below their 20-year average on at least one metric")st.caption(f"Last updated: {datetime.now().strftime('%Y-%m-%d %H:%M')} • Free data via Financial Modeling Prep")
